...
def main(
    ...
    for scenario_id in range(max_cases):
        ...
        for inf_id in range(max_infs):
            ...
            doctor_dialogue = _intake_say(inf_type, intake_agent, current_observation)

            if doctor_dialogue is None:
                print("IntakeAgent returned None, skipping this case.")
                break

            print(f"Intake [{progress}%]: {doctor_dialogue}")
            time.sleep(1.0)

            # ---------- Intake 是否完成信息采集 ----------  

            if is_intake_complete(doctor_dialogue):
                print("Intake complete signal detected. Ending consultation.")
                intake_agent.write_structured_patient_info(case_store)
...
def _intake_say(inf_type, intake_agent, current_observation):
    if inf_type == "human_doctor":
        doctor_dialogue = input("\nDoctor: ") # human input
    else:
        doctor_dialogue = intake_agent.inference(current_observation)

        if doctor_dialogue is not None:
            intake_agent.add_hist(
                f"Patient: {current_observation}\nDoctor: {doctor_dialogue}"
            )
    return doctor_dialogue
